 ...
 *
 */
if (!class_exists('SOALI_OPTIONS_Setup')) {
    class SOALI_OPTIONS_Setup
    {
        ...
        public static function setup()
        {
            ...
            if (class_exists('SOALI_OPTIONS_Options') && !empty(self::$args['admin_options'])) {
                foreach (self::$args['admin_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Options::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Customize_Options') && !empty(self::$args['customize_options'])) {
                foreach (self::$args['customize_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Customize_Options::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Metabox') && !empty(self::$args['metabox_options'])) {
                foreach (self::$args['metabox_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Metabox::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Nav_Menu_Options') && !empty(self::$args['nav_menu_options'])) {
                foreach (self::$args['nav_menu_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Nav_Menu_Options::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Profile_Options') && !empty(self::$args['profile_options'])) {
                foreach (self::$args['profile_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Profile_Options::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Taxonomy_Options') && !empty(self::$args['taxonomy_options'])) {
                ...
                foreach (self::$args['taxonomy_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Taxonomy_Options::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Widget') && class_exists('WP_Widget_Factory') && !empty(self::$args['widget_options'])) {
                ...
                foreach (self::$args['widget_options'] as $key => $value) {
                    if (!isset(self::$inited[$key])) {
                        ...
                        self::$inited[$key] = true;
                        $wp_widget_factory->register(SOALI_OPTIONS_Widget::instance($key, $value));
            ...
            if (class_exists('SOALI_OPTIONS_Comment_Metabox') && !empty(self::$args['comment_options'])) {
                foreach (self::$args['comment_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Comment_Metabox::instance($key, $params);
            ...
            if (class_exists('SOALI_OPTIONS_Shortcoder') && !empty(self::$args['shortcode_options'])) {
                foreach (self::$args['shortcode_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        SOALI_OPTIONS_Shortcoder::instance($key, $params);

                    }
                }

                // Once editor setup for gutenberg and media buttons
                if (class_exists('SOALI_OPTIONS_Shortcoder') && !empty(self::$shortcode_instances)) {
                    foreach (self::$shortcode_instances as $instance) {
                        if (!empty($instance['show_in_editor'])) {
                            SOALI_OPTIONS_Shortcoder::once_editor_setup();
                            break;
                        }
                    }
                }
            ...
            do_action('soali_options_loaded');
        ...
        // Create woocommerce metabox options
        ...
        public static function include_plugin_file($file, $load = true)
        {
            ...
            $path = '';
            $file = ltrim($file, '/');
            $override = apply_filters('soali_options_override', 'soali_options-override');
        ...
        public static function includes()
        {
            ...
            $fields = apply_filters('soali_options_fields', array(
                'accordion',
                'background',
                'backup',
                'border',
            ...
            if (!empty($fields)) {
                foreach ($fields as $field) {
                    if (!class_exists('SOALI_OPTIONS_Field_' . $field) && class_exists('SOALI_OPTIONS_Fields')) {
        ...
        public static function add_admin_enqueue_scripts()
        {
            ...
            if (!self::$enqueue) {
                ...
                if (!empty(self::$args['woocommerce_metabox_options'])) {
                    foreach (self::$args['woocommerce_metabox_options'] as $argument) {
                        if (in_array($wpscreen->post_type, ['product'])) {
                            self::$enqueue = true;
                        }
                    }
                }
                ...
                if ($wpscreen->id === 'tools_page_soali_options-welcome') {
                    self::$enqueue = true;
                }
        ...
        public static function enqueue_scripts()
        {
            ...
            if (apply_filters('soali_options_fa4', false)) {
                wp_enqueue_style('soali_options-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome.min.css', array(), '4.7.0', 'all');
            } else {
                wp_enqueue_style('soali_options-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/all.min.css', array(), '5.15.5', 'all');
                wp_enqueue_style('soali_options-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/v4-shims.min.css', array(), '5.15.5', 'all');
            }
            ...
            // Main style
            wp_enqueue_style('soali_options', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');

            // Main RTL styles
            if (is_rtl()) {
                wp_enqueue_style('soali_options-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), array(), self::$version, 'all');
            }
            ...
            // Main variables
            wp_localize_script('soali_options', 'soali_options_vars', array(
                'color_palette' => apply_filters('soali_options_color_palette', array()),
                'i18n' => array(
                    'confirm' => esc_html__('Are you sure?', 'soali_options'),
                    'typing_text' => esc_html__('Please enter %s or more characters', 'soali_options'),
                    'searching_text' => esc_html__('Searching...', 'soali_options'),
                    'no_results_text' => esc_html__('No results found.', 'soali_options'),
                ),
            ));
            ...
            do_action('soali_options_enqueue');
        ...
        public static function add_typography_enqueue_styles()
        {
            ...
            if (!empty(self::$webfonts)) {
                ...
                if (!empty(self::$webfonts['enqueue'])) {
                    ...
                    wp_enqueue_style('soali_options-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), array(), null);
                ...
                if (!empty(self::$webfonts['async'])) {
                    ...
                    wp_enqueue_script('soali_options-google-web-fonts', esc_url('//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'), array(), null);

                    wp_localize_script('soali_options-google-web-fonts', 'WebFontConfig', array('google' => array('families' => $fonts)));
        ...
        public static function add_admin_body_class($classes)
        {
            ...
            if (apply_filters('soali_options_fa4', false)) {
                $classes .= 'soali_options-fa5-shims';
            }
        ...
        public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
        {
            ...
            if (!empty($field['_notice'])) {
                ...
                $field = array();
                $field['content'] = esc_html__('Oops! Not allowed.', 'soali_options') . ' <strong>(' . $field_type . ')</strong>';
                $field['type'] = 'notice';
                $field['style'] = 'danger';
            ...
            $is_pseudo = (!empty($field['pseudo'])) ? ' soali_options-pseudo-field' : '';
            ...
            if (!empty($field['dependency'])) {
                ...
                $visible = (!empty($depend_visible)) ? ' soali_options-depend-visible' : ' soali_options-depend-hidden';

            }

            // These attributes has been sanitized above.
            echo '<div class="soali_options-field soali_options-field-' . $field_type . $is_pseudo . $class . $visible . '"' . $depend . '>';

            if (!empty($field_type)) {

                if (!empty($field['title'])) {
                    echo '<div class="soali_options-title">';
                    echo '<h4>' . $field['title'] . '</h4>';
                    echo (!empty($field['subtitle'])) ? '<div class="soali_options-subtitle-text">' . $field['subtitle'] . '</div>' : '';
                    echo '</div>';
                }

                echo (!empty($field['title'])) ? '<div class="soali_options-fieldset">' : '';

                $value = (!isset($value) && isset($field['default'])) ? $field['default'] : $value;
                $value = (isset($field['value'])) ? $field['value'] : $value;

                $classname = 'SOALI_OPTIONS_Field_' . $field_type;

                if (class_exists($classname)) {
                    $instance = new $classname($field, $value, $unique, $where, $parent);
                    $instance->render();
                } else {
                    echo '<p>' . esc_html__('Field not found!', 'soali_options') . '</p>';
                }
...
SOALI_OPTIONS_Setup::init(__FILE__, false);
...
if (!class_exists('SOALI_OPTIONS')) {
    class SOALI_OPTIONS extends SOALI_OPTIONS_Setup
    {
    }
}
